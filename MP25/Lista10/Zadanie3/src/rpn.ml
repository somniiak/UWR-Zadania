...
(* "(3 + 8) - 4 * (2 - 1)" |> parse |> of_ast [] |> stack_size;; *)

let stack_size (p : prog) : int =
  let rec aux prog curr max_height =
    match prog with
    | [] -> max_height
    | cmd :: rest ->
      let pop, push =
        match cmd with
        | PushInt _ | PushBool _ | PushUnit -> (0, 1)
        | PushPair -> (2, 1)
        | Binop _ -> (2, 1)
        | CndJmp _ -> (1, 0)  (* bo warunek schodzi, a skok może różnie wpływać, ale maks śledzimy lokalnie *)
        | Fst -> (1, 1)
        | Snd -> (1, 1)
        | IsPair -> (1, 1)
        | Load _ -> (0, 1)
        ...
        | CleanUp -> (0, 0)
      in
      let curr' = curr - pop + push in
      if curr' < 0 then failwith "stack underflow"
      else aux rest curr' (max max_height curr')
  in
  aux p 0 0
