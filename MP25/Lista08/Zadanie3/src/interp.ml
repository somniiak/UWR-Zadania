...
let rec rename_expr_env (path : string) (env : string SM.t) (e : expr) : expr =
  let fresh path = "#" ^ path in
  match e with
  | Int n -> Int n
  | Bool b -> Bool b
  | Unit -> Unit
  ...
  | Binop(op, l, r) ->
    Binop(op,
    rename_expr_env (path ^ "L") env l,
    rename_expr_env (path ^ "R") env r)
  | If(b, t, e) ->
    If(
      rename_expr_env (path ^ "B") env b,
      rename_expr_env (path ^ "T") env t,
      rename_expr_env (path ^ "E") env e
    )
  | Let(x, e1, e2) ->
    let new_x = fresh path in
    let new_env = SM.add x new_x env in
    Let(
      new_x,
      rename_expr_env (path ^ "L") new_env e1,
      rename_expr_env (path ^ "R") new_env e2
    )
  | Fun(x, e) ->
    let new_x = fresh path in
    let new_env = SM.add x new_x env in
    Fun(new_x, rename_expr_env (path ^ "B") new_env e)
  | Funrec(f, x, e) ->
    let new_f = fresh (path ^ "F") in
    let new_x = fresh (path ^ "X") in
    let new_env = SM.add f new_f (SM.add x new_x env) in
    Funrec(new_f, new_x, rename_expr_env (path ^ "B") new_env e)
  | App(e1, e2) ->
    App(
      rename_expr_env (path ^ "L") env e1,
      rename_expr_env (path ^ "R") env e2
    )
  | Pair(e1, e2) ->
    Pair(
      rename_expr_env (path ^ "L") env e1,
      rename_expr_env (path ^ "L") env e2
    )
  | Fst e -> Fst(rename_expr_env (path ^ "F") env e)
  | Snd e -> Fst(rename_expr_env (path ^ "S") env e)
  | IsPair e -> IsPair(rename_expr_env (path ^ "I") env e)
  | Match(_e1, _x, _y, _e2) -> failwith "Not implemented"
    

let rename_expr (e : expr) : expr =
  rename_expr_env "" SM.empty e
