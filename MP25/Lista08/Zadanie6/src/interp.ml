...
let eval_op (op : bop) (val1 : value) (val2 : value) : value =
  ...
  | Add,   _,        _        -> failwith "Dodawanie jeblo"
  | Mult,   _,        _        -> failwith "Mnozenie jeblo"
  | Mod,   _,        _        -> failwith "Modulo jeblo"
  | _,    _,        _        -> failwith "Operatory sie wyjebaly"
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | If (b, t, e) ->
      (match eval_env env b with
        ...
        | _ -> failwith "If sie wyjebalo")
  | Var x ->
     (match M.find_opt x env with
       | Some v -> v
       | None -> failwith "Var sie wyjebal")
  | Let (x, e1, e2) ->
  ...
  | Fst e ->
      (match eval_env env e with
        | VPair (v1, _) -> v1
        | _ -> failwith "Fst sie wyjebalo")
  | Snd e ->
      (match eval_env env e with
        | VPair (_, v2) -> v2
        | _ -> failwith "Snd sie wyjebalo")
  | Match (_e1, _x, _y, _e2) ->
  ...
  | LetRec (f, x, e1, e2) ->
    (* Create a recursive closure for the function *)
    let rec_fun = VRecClosure (f, x, e1, env) in
    (* Add the recursive function to the environment before evaluating e2 *)
    eval_env (M.add f rec_fun env) e2

(*
dune exec ./_build/default/bin/main.exe examples/fib_fast.fun
*)
